...
module KindleExtractor
  RSpec.describe WritesAnkiImportFile do
    include FakeFS::SpecHelpers

    BOOK_TITLE = 'frog'
    let(:writer) { described_class.new }

    let(:highlight) { double("highlight", book_title: BOOK_TITLE,
                        content: 'it jumps',
                        author: 'John Doe',
    ...
    describe "how it outputs a highlight as an Anki flashcard" do
      context "given a highlight from a book titled '#{BOOK_TITLE}'" do
        it "writes the highlight to a file named '#{BOOK_TITLE}.txt'" do
          expect(File).to receive(:open).with(/#{BOOK_TITLE}.txt$/, 'a')
          writer.write_out(highlight)
        end

        it "puts the content on the 'front' of the card" do
          file = double("file")
          expect(File).to receive(:open).with(/#{BOOK_TITLE}.txt$/, 'a').and_yield(file)
          expect(file).to receive(:puts).with(/^it jumps/)
          writer.write_out(highlight)
        end
      end
      ...
      it "accepts an output directory to write files to" do
        ...
        writer.write_out(highlight)
        expect(File.exist?( File.expand_path(File.join("~/Desktop/to_import_into_anki","#{BOOK_TITLE}.txt")) )).to be true
      end
    end
  end
end
